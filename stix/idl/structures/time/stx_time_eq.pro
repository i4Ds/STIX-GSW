;+
; :description:
;    this function checks of the left time is equal the right time
;
; :categories:
...
;    time_r = stx_construct_time(time='2012-12-12 5:00')
;    print, stx_time_eq(time_l,time_r)
;    
; :history:
...
; 
;-
function stx_time_eq, left, right
  if ~ppl_typeof(left,compareto="stx_time",/raw) || ~ppl_typeof(right,compareto="stx_time",/raw) then return, 0
  return, left.value.MJD eq right.value.MJD AND left.value.TIME eq right.value.TIME 
end
