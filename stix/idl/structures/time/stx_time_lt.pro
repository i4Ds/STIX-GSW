;+
; :description:
;    this function checks of the left time is lover than the right time
;
; :categories:
...
;    print, stx_time_lt(time_l,time_r)
;    
; :history:
...
; 
;-
function stx_time_lt, left, right
  if ~ppl_typeof(left,compareto="stx_time",/raw) || ~ppl_typeof(right,compareto="stx_time",/raw) then return, 0
  return, left.value.MJD lt right.value.MJD OR (left.value.MJD eq right.value.MJD AND left.value.TIME lt right.value.TIME) 
end
