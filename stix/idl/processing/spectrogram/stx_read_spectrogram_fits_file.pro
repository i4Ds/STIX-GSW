pro stx_read_spectrogram_fits_file, fits_path, time_shift, primary_header = primary_header, data_str = data, data_header = data_header, control_str = control, $
  ...
  if ~keyword_set(keep_short_bins) then begin
    ...
    counts_for_time_bin=total(shifted_counts[1:10,*],1)
    ...
    counts_short_removed = shifted_counts[*,idx_long]
    counts_err_short_removed  = shifted_counts_err[*,idx_long]
    triggers_short_removed = shifted_triggers[idx_long]
    triggers_err_short_removed = shifted_triggers_err[idx_long]
    ...
    counts_err =  shifted_counts_err
    triggers = shifted_triggers
    triggers_err=   shifted_triggers_err
