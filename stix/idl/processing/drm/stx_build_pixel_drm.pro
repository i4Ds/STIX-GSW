...
function stx_build_pixel_drm, ct_energy_edges, pixel_mask, _extra = _extra
...
;the input pixel mask must be a 12 x 32 element of 1s and 0s repenting the pixels from each detector to be used   
  ...
  drm = stx_build_drm( ct_energy_edges, _extra=_extra )
  ...
  drm.area *= scale_factor
  drm.smatrix *= scale_factor
  drm.eloss_mat *= scale_factor
  drm.pls_ht_mat *= scale_factor
