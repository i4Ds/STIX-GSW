...
;    offset - default of channel 700.0 of 4096
...
pro stx_bkg_continuum_mdl, edg2, out, grimm_howard_par = grimm_howard_par, $
  ...
  coef = [0.0, 0.1] ;offset and gain (ev/bin)
  if keyword_set( gain_cf ) then begin ;gain_cf are the offset and gain (ev/bin) respectively
    mm = minmax( edg2 )
    nchan = (mm[1]-mm[0])/ gain_cf[1]
    new_edg = findgen(nchan+1) * gain_cf[1] + gain_cf[0]
    ssw_rebinner, out, edg2, newout, new_edg
    edg2 = new_edg
    out  = newout
