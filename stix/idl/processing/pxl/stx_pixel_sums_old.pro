...
;         to implement when we can no lonber have 4 d pixel data arrays. Ultimately they must be 3 d,

;         one for the cell index, one for the detector, one for the pixel. The cell index is has unique time bin

;         and energy bin identifier. Not an array of time and energy cells
...
function stx_pixel_sums_old, pixel_data, sumcase, fsw=fsw
  ...
  ncells = (size(pixel_data.counts))[1]



  ; group by time-energy-detector and pixel row, Ncells x 4 x 3 x inputs

  pixels = reform(pixel_data.counts, ncells, 4, 3, n_inputs)

  summed_pixels = n_elements( indices ) eq 1 ? reform(pixels[*, *, indices,*]) : total( pixels[*, *, indices,* ], 3 )
