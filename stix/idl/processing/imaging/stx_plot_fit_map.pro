...
pro stx_plot_fit_map, in_map, chi2=chi2, this_window=this_window
...
obs_vis_amp  = abs(obs_vis)
obs_sigamp = in_map.OBS_VIS.sigamp  ; Error on observed visibility amplitudes
pred_vis_amp = abs(pred_vis)

;;*********** Phases

obs_vis_phase  = atan(imaginary(obs_vis), float(obs_vis)) * !radeg
obs_sigphase   = f_div(obs_sigamp, obs_vis_amp) * !radeg
pred_vis_phase = atan(imaginary(pred_vis), float(pred_vis)) * !radeg
...
plot, xx, obs_vis_phase, /nodata, xrange=xrange, /xst, xtickinterval=1, xminor=-1, $
...
errplot, xx, (obs_vis_phase - obs_sigphase > !y.crange[0]), (obs_vis_phase + obs_sigphase < !y.crange[1]), $
  width=0, thick=thick, color=color1
oplot, xx, obs_vis_phase, psym=7, thick=thick, symsize=symsize
oplot, xx, pred_vis_phase, psym=4, thick=thick, symsize=symsize, color=color2
...
oplot, xx, (obs_vis_phase - pred_vis_phase)/obs_sigphase, psym=1, symsize=symsize, color=color0, thick=thick_res


set_viewport,0.57, 0.95, 0.27, 0.82

plot, xx, obs_vis_amp, /nodata, xrange=xrange, /xst, xtickinterval=1, xminor=-1, $
  title='VISIBILITY AMPLITUDE FIT', XTICKFORMAT="(A1)", $
  xtitle='', ytitle=units_amp, charsize=charsize, thick=thick, yrange=[0,max(obs_vis_amp + obs_sigamp)*4/3], /yst, /noe

; draw vertical dotted lines at each detector boundary
for i=1,10 do oplot, i+[0,0], !y.crange, linestyle=1


errplot, xx, (obs_vis_amp - obs_sigamp > !y.crange[0]), (obs_vis_amp + obs_sigamp < !y.crange[1]), $
  width=0, thick=thick, COLOR=color1
oplot, xx, obs_vis_amp, psym=7, thick=thick, symsize=symsize
oplot, xx, pred_vis_amp, psym=4, thick=thick, symsize=symsize, color=color2
...
oplot, xx, xx*0., linestyle=1, color=color2, thick=thick
oplot, xx, (obs_vis_amp - pred_vis_amp)/obs_sigamp, psym=1, symsize=symsize, color=color0, thick=thick_res
