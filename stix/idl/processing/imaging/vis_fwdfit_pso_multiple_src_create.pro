...
FUNCTION VIS_FWDFIT_PSO_MULTIPLE_SRC_CREATE, vis, configuration, aux_data = aux_data
...
endelse
  ...
  if n_circle gt 0 then begin
    src_circle = vis_fwdfit_pso_circle_struct_define(vis, dummy0)
    src_circle = cmreplicate(src_circle, n_circle)
  endif
  
  if n_ellipse gt 0 then begin
    src_ellipse = vis_fwdfit_pso_ellipse_struct_define(vis, dummy0)
    src_ellipse = cmreplicate(src_ellipse, n_ellipse)
  endif
  
  if n_loop gt 0 then begin
    src_loop = vis_fwdfit_pso_loop_struct_define(vis, dummy0)
