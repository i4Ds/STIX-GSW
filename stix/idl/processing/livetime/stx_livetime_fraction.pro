...
;-
function stx_livetime_fraction, triggergram,  det_select, tau_array = tau_array,  error=error
...
default, tau_array, 9.6e-6 + fltarr(ntrig)  ;9.6 microseconds processing time per event
ndt = n_elements( triggergram.t_axis.duration )
duration = transpose( rebin( triggergram.t_axis.duration, ndt, ntrig ))
tau_rate =    triggergram.triggerdata * rebin( tau_array, ntrig, ndt ) / duration
livetime_fraction = exp( -tau_rate )
