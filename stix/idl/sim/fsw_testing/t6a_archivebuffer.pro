...
;openr, lun, 'D:\Dropbox (STIX)\FSW_Test_Data\Data\Published\20170124_221151\ESC\20170309\T6a\T6a_rb_processing.txt' , /GET_LUN

openr, lun, 'D:\Dropbox (STIX)\FSW_Test_Data\Data\Published\20170124_221151\ESC\20170330\T6a_rb_processing_ver2_reformated.txt' , /GET_LUN
...
foreach line, lines do begin
  ...
  triggers =  {stx_fsw_triggers}
  triggers.relative_time_range = entries[0].RELATIVE_TIME_RANGE

  
  foreach trigger_str, triggers_data, idx do begin
    HEX2DEC, trigger_str, count, /quiet
    triggers.triggers[idx] = count
  endforeach
  
  
  all_triggers->add, triggers
...
endforeach
 ...
 all_triggers = all_triggers->toarray()
 
 all_fsw_entries=list()
 all_fsw_triggers=list()
 
 restore, filename='D:\Dropbox (STIX)\FSW_Test_Data\Data\Raw\v20170124\T1a\T1a_dss-fsw.sav', /ver, /relaxed_structure_assignment
 ...
 foreach fsw_g, abg do begin
  ...
  all_fsw_triggers->add, fsw_g.Triggers, /extract
 endforeach
 
 all_fsw_entries = all_fsw_entries->toarray()
 all_fsw_triggers = all_fsw_triggers->toarray()
 ...
 assert_equals, all_fsw_triggers.triggers-all_triggers.triggers, 0
 ...
 all_table = string(all_entries.relative_time_range[0]) + string(all_entries.relative_time_range[1]) + string(fix(all_entries.pixel_index)) + string(fix(all_entries.detector_index)) + string(fix(all_entries.energy_science_channel)) + string(fix(all_entries.counts))
 all_fsw_table = string(all_fsw_entries.relative_time_range[0]) + string(all_fsw_entries.relative_time_range[1]) + string(fix(all_fsw_entries.pixel_index)) + string(fix(all_fsw_entries.detector_index)) + string(fix(all_fsw_entries.energy_science_channel)) + string(fix(all_fsw_entries.counts))

 all_fsw_table = all_fsw_table[sort(all_fsw_table)]

 all_table = all_table[sort(all_table)]
 
 
 assert_equals, n_elements(all_entries), n_elements(all_fsw_entries)
 
 mis = where(all_fsw_table ne all_table, ec)
