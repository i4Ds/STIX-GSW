...
;
;-
function stx_make_l1_ql_lightcurve_fits, fits_path, base_directory
  ...
  filename = 'solo_l1_stix-lightcurve_'+trim(time_stamp(obt_beg))+'_V'+trim(tstamp)+'.fits'
...
;
;-
function stx_make_l1_ql_background_fits, fits_path, base_directory
  !null = mrdfits(fits_path, 0, primary_header)
  control = mrdfits(fits_path, 1, control_header)
  data = mrdfits(fits_path, 2, data_header)
  ...
  filename = 'solo_l1_stix-background_'+trim(time_stamp(obt_beg))+'_V'+trim(tstamp)+'.fits'
...
;
;-
function stx_make_l1_ql_variance_fits, fits_path, base_directory
  !null = mrdfits(fits_path, 0, primary_header)
  control = mrdfits(fits_path, 1, control_header)
  data = mrdfits(fits_path, 2, data_header)
  ...
  filename = 'solo_l1_stix-variance_'+trim(time_stamp(obt_beg))+'_V'+trim(tstamp)+'.fits'
