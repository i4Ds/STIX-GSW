...
;      in sum scheme 1 (summing large pixels in top row, large pixels in bottom
;      row and small pixels to a total of 3 'pixels').
...
;    17-Dec-2015 - Roman Boutellier (FHNW),
;-
function stx_detector_summed_1_object_graphics::init, _extra=extra
  ...
  ; Create the graphic object for the small pixels
  s_complete = obj_new('IDLgrPolygon', transpose([[sxr*x[0]-2,sxr*x[1]+1,sxr*x[2]+1,sxr*x[3]-2,sxr*x[4]-2],[sy]]), name="s1", color=rgb)
  ; Add the small pixel and the lines of the small pixel to self
  self->add, obj_new('IDLgrPolyline', *s_complete.data, name="bs1")
  self->add, s_complete
  ...
  ; Create the graphic objects for the big pixel in the top row
  t_complete = obj_new('IDLgrPolygon', transpose([[x[0]-2,x[1]+1,x[2]+1,x[3]-2,x[4]-2],[lyr*y]]), name="t1", color=rgb )
  ; Add the top row big pixel and its line
  self->add, obj_new('IDLgrPolyline', *t_complete.data, name="bt1")
  self->add, t_complete
  
  ; Create the graphic objects for the big pixel in the bottom row
  b_complete = obj_new('IDLgrPolygon', transpose([[xb[0]-2,xb[1]-2,xb[2]+1,xb[3]+1,xb[4]-2],[-lyr*yb]]), name="b1", color=rgb )
  ; Add the 4 bottom row big pixels and their lines
  self->add, obj_new('IDLgrPolyline', *b_complete.data, name="bb1")
  self->add, b_complete
...
;    17-Dec-2015 - Roman Boutellier (FHNW),
;-
function stx_detector_summed_1_object_graphics::_overloadPrint
...
;    17-Dec-2015 - Roman Boutellier (FHNW),
;-
pro stx_detector_summed_1_object_graphics::_update_border
...
;    17-Dec-2015 - Roman Boutellier (FHNW),
;-
pro stx_detector_summed_1_object_graphics::_update_plot
   compile_opt idl2, hidden
...
;    17-Dec-2015 - Roman Boutellier (FHNW),
;-
pro stx_detector_summed_1_object_graphics::_update_data
   compile_opt idl2, hidden
   
   if ~isa(self.uvalue) || ~isa(*self.uvalue, /array, /number) then return
   
   bottom_elements = ["bplot","bs1","bb1","s1","b1"]
   ...
   foreach name, ["t1","b1","s1"], index do begin
...
;    17-Dec-2015 - Roman Boutellier (FHNW),
;-
pro stx_detector_summed_1_object_graphics::_getPlotData, data, x=x, y=y
  compile_opt idl2, hidden
...
;    17-Dec-2015 - Roman Boutellier (FHNW),
;-
function stx_detector_summed_1_object_graphics::_getColor, _value
  compile_opt idl2, hidden
  
  value = _value
  value[2] *= self.smallpixelboost
...
pro stx_detector_summed_1_object_graphics::cleanup
...
;    17-Dec-2015 - Roman Boutellier (FHNW),
;-
pro stx_detector_summed_1_object_graphics::SetProperty, _EXTRA=ex, $
...
pro stx_detector_summed_1_object_graphics__define
  compile_opt idl2
  
  define = {stx_detector_summed_1_object_graphics, $
             bordercolor : 255, $
             borderthick : 2, $
             linecolor : 255, $
             linethick : 2, $
