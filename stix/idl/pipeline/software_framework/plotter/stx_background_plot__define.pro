;+
; :file_comments:
;   The background plot object. It can be used to plot STIX background lightcurves.
...
;
; :history:
;    02-Apr-2015 - Roman Boutellier (FHNW), Initial release
;-
...
;    02-Apr-2015 - Roman Boutellier (FHNW), Initial release
;-
function stx_background_plot::init
...
;+
; :description:
;    Plots the background data of a stx_lightcurve object.
;
; :Params:
...
;-
pro stx_background_plot::plot, lightcurve_object, dimension=dimension, position=position, current=current, add_legend=add_legend, overplot=overplot, _extra=extra
...
pro stx_background_plot::append_data, lightcurve_object, histogram=histogram
  ; Show an error message in case the input is not a lightcurve object
  if(~ppl_typeof(lightcurve_object, compareto='stx_lightcurve', /raw)) then message, 'Invalid input: Only stx_lightcurve allowed'
...
function stx_background_plot::_get_styles
  ; Get the default styles
  default_styles = stx_line_plot_styles(/background)
  return, default_styles
end

pro stx_background_plot__define
  compile_opt idl2
  
  define = {stx_background_plot, $
