...
;-
pro stx_software_framework, dss=dss, fsw=fsw, run_simulation=run_simulation, scenario_name=scenario_name, scenario_file=scenario_file
  ...
  if (keyword_set(run_simulation)) then begin
    ...
    res = dss->getdata(scenario_name=scenario_name, scenario_file=scenario_file)
    ...
    for time_bin = 0L, 1000L-1 do begin
      ; Iteratively request data. This is where the Rate Control Regime will be set (in future)
      ds_result_data = dss->getdata(output_target='stx_ds_result_data', time_bin=time_bin, scenario_name=scenario_name, rate_control_regime=0)
