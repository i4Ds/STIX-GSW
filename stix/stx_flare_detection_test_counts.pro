...
;
; :history:
;    07-Mar-2017 - ECMD (Graz), Initial release
;
;-
function stx_flare_detection_test_counts, background = background,short_timescale_s =short_timescale_s, long_timescale_s = long_timescale_s, $
  ...
  ;the loaction of the counts peak as a fraction of the event time
  ...
  for baseline = 0,1 do begin
    for eband = 0,1 do begin
      for event = 0,8 do begin
        ...
        seed = 1000l
        random_bg = long(randomu(seed, 31l*(preevent+nbins), poisson=background_expectation[eband]))
        ...
        current_counts[*, eband] = median_background
        ...
        ;the counts and background for the completer event are added to the full arrays
