...
;       10-mar-2014 - richard.schwartz@nasa.gov, added keyword inheritance to pass through to stx_build_drm
;-
...
;	ATTENUATOR - passed to stx_build_drm, set to 1 for attenuator, 0 for no attenuator, defaults to 0
;   
; :params:
...
;       OBJNAME    - object variable, this is a dummy name but it must be a variable name and not a string, 
;           it will be returned as an object.
...
pro stx_sim_ospex_demo, filename = filename, peaktime = peaktime, plotint = plotint, objname = objname, $
  seed1 = seed1, noplot = noplot, nosave = nosave, folder = folder, _extra = _extra
...
print, 'CALLING stx_build_drm to calculate STIX DETECTOR RESPONSE MATRIX'

stx_drm = stx_build_drm(ct_edges1, _extra = _extra)
